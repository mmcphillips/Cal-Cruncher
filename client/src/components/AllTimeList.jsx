...
height:30px;`
const AllTimeList = ({allIntake, remove, modify, state}) => {
  ...
  if (state.todayTotal < state.recommended) {
    ...
    <DailyListContainer>
      <Header>
        ...
        <div>
         <SP> Average Net:{state.recommended}</SP>
        </div>
           <SP>
             Average Net:
             <GreenDiv>-{Math.abs(state.recommended- state.allTimeTotal)}
          </GreenDiv>
              cal
          </SP>
      </Header>
    ...
    {allIntake.map((item) => <ConsumptionItem item={item} remove={remove} modify={modify}/>)}
      <Spacer></Spacer>
    </Tb>
  </DailyListContainer>
  )}
